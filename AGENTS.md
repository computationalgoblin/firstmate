...
A `state/<id>.turn-ended` notification plus an unknown current-state result proves neither exit nor a wedge and authorizes neither relaunch nor a failure declaration; `stuck-crewmate-recovery` owns the positive-evidence progression.
